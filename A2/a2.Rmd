...
When explorint the price relationship with factors, we can see that the neighbourhood has an effect on price. 
...
With `geosphere` package we introduce  a numerical value that can represent the location of the apartment other than the factorized neighbourhood to be later used for the "distance to the center" feature. Considering that a large par of our listings is located in the first district, **Centro Storico**, we introduced a dummy variable to better facilitate CART based models. 
...
(some negative correlations does not seem reasonable)
...
We create a holdout set as a random 15% of our cleaned data table, resulting approximately 2000 observations for the holdout. 
...
Our modelsummary confirms the significants of variables through correlations. Factorizing the neighborhoods seems to improve the preformance. 
...
Random forest best result has 5 predictors when selecting model4 formula (with 15 variables).
...
rf_model_var_imp_df <-
  data.frame(varname = imp = rf_model_var_imp) %>%
  arrange(desc(imp)) %>%
